        ...
        while True:
            ...
            for name,code in ir.IRListener().buttons.items():
                if hex_signal == hex(code):
                    ...
                    if name == "1":
                        sio.emit('pollResp','Up')
                    elif name == "2":
                        sio.emit('pollResp','Wiggle')
                    elif name == "3":
                        sio.emit('pollResp','Down')
                    elif name == "4":
                        sio.emit('pollResp','remove')
                    elif name == "play_pause":
                        sio.emit('startPoll', ("3", False, "", [{"answer":"banana","weight":1,"color":"#ff0000"}, {"answer":"b","weight":1,"color":"#00ff00"}, {"answer":"c","weight":1,"color":"#0000ff"}], {}, 1, [], [], [], [], False))
                        print('message sent')
                    time.sleep(0.5)
                    break
